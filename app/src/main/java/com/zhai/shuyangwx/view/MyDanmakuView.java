package com.zhai.shuyangwx.view;
...
//                .setCacheStuffer(new BackgroundCacheStuffer(), null)  // 绘制背景使用BackgroundCacheStuffer
     ...
     * @param isServer 是不是服务器发送
     */
    public void addDanmaku(String text, boolean isSelf,boolean isServer) {
        ...
        danmaku.textSize = PlayerUtils.sp2px(getContext(), 20);
        if(isServer){
     ...
     * 发送自定义弹幕
     */
    public void addDanmakuWithDrawable(String str) {
        ...
        // for(int i=0;i<100;i++){
        // }
        Drawable drawable = ContextCompat.getDrawable(getContext(), R.mipmap.ic_logo_qq);
        int size = PlayerUtils.dp2px(getContext(), 20);
        drawable.setBounds(0, 0, size, size);

//        danmaku.text = "这是一条弹幕";
        danmaku.text = createSpannable(drawable,str);
//        danmaku.padding = 5;
        danmaku.priority = 0;  // 可能会被各种过滤器过滤并隐藏显示
        danmaku.isLive = false;
        danmaku.setTime(getCurrentTime());
